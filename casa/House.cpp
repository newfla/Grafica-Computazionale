...
void Mesh::House::draw(){
            for(short int i = 0; i < faces.size(); i++)
            {
                ...
                glBegin(GL_POLYGON);
                    ...
                    cout<<normal[0]<<" __ "<<normal[1]<<" __ "<<normal[2]<<endl;
                    glNormal3fv(normal);
...
vector<Mesh::Point> Mesh::House::buildPoints(){
    vector<Point> points;
    points.push_back(*(new Point(0,0,0)));
    points.push_back(*(new Point(0.2,0,0)));
    points.push_back(*(new Point(0.2,0.2,0)));
    points.push_back(*(new Point(0.1,0.3,0)));
    points.push_back(*(new Point(0,0.2,0)));
    points.push_back(*(new Point(0,0,-0.4)));
    points.push_back(*(new Point(0.2,0,-0.4)));
    points.push_back(*(new Point(0.2,0.2,-0.4)));
    points.push_back(*(new Point(0.1,0.3,-0.4)));
    points.push_back(*(new Point(0,0.2,-0.4)));
    return points;
